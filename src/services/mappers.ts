...
export function dbToEquipment(row: DbEquipamento): Equipment {
  const dadosTecnicos = (row.dados_tecnicos ?? {}) as Record<string, string | number | boolean | null>;
  ...
  const eq: Equipment = {
    ...
    status: row.status,
    qrCode: emptyToUndef(row.qr_code),
    qrcode: emptyToUndef(row.qr_code),
  ...
  for (const [key, val] of Object.entries(dadosTecnicos)) {
    if (val !== null && val !== undefined && val !== '') {
      (eq as unknown as Record<string, unknown>)[key] = val;
    }
  }
...
export function equipmentToDb(eq: Partial<Equipment>): Record<string, unknown> {
  ...
  if (eq.dadosTecnicos) {
    Object.assign(dadosTecnicos, eq.dadosTecnicos);
  }
  for (const key of Object.keys(eq as unknown as Record<string, unknown>)) {
    if (!COMMON_FIELDS.has(key)) {
      const val = (eq as Record<string, unknown>)[key];
      if (val !== undefined) {
        dadosTecnicos[key] = val ?? null;
      }
    }
  }
  row.dados_tecnicos = dadosTecnicos;
